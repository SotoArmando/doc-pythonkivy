...
class Inicio(Screen):
    def __init__(self, **kwargs):
        ...
        programparent = RelativeLayout(size_hint_x = .95 ,size_hint_y = .5,y = Window.height-dp(79)-(Window.height*.5), pos_hint = {'center_x':.5})
        ...
        for i in range(7): 
            ...
            widget_parent_background.add_widget(Image(source = color + '5.png', keep_ratio = False, allow_stretch = True))
            widget_parent = GridLayout(cols = 1)
            widget_parent.add_widget(ImageButton(on_release = lambda x: self.Animation(x.parent.parent.parent.parent),size_hint_y = None , height = '60px', source = asset + img[i] + '.png'))
            widget_parent.add_widget(Button(on_release = lambda x: self.Animation(x.parent.parent.parent.parent),background_normal = color + 'None.png', text = C4+text[i], markup = True))
