...
color = patch + '/activities/colors/'
font = patch + '/fonts/'
class ImageButton(ButtonBehavior, Image): pass
class Projectil(Scatter):
    z = NumericProperty(0)# entre 1 a 100
    aceleracion_gravedad = NumericProperty(0.098) #pixeles
    aceleracion = NumericProperty(0.05)
    velocidad = NumericProperty(0.2)
    def __init__(self,**kwargs):
        super(Projectil, self).__init__(**kwargs)
        direccion = NumericProperty()
        velocidad = NumericProperty(2)
        self.size_hint = (None,None)
        self.size = (64,64)
        self.add_widget(Image(opacity = .95,source = color + "7.png" ,keep_ratio = False, allow_stretch = True ))
        with self.canvas:
            Color(.5,0,0,1)
            Rectangle(size = self.size, pos = self.pos)
        Clock.schedule_interval(lambda x: self.go(), 1.0/60.0)
    def on_z(self,*args):
        if self.velocidad > 0:
            self.aceleracion = self.aceleracion - self.aceleracion_gravedad
            self.velocidad =  self.velocidad + self.aceleracion
            self.scale = (self.z/1000.0) + self.scale
        else:
            self.aceleracion = self.aceleracion + self.aceleracion_gravedad
            self.velocidad =  self.velocidad + self.aceleracion
            self.scale = (self.z/1000.0 * -1) + self.scale
        print "z:",self.z , "velocidad:", self.velocidad
      
    def go(self,*args): 
        
        self.z = self.z + self.velocidad
...
class TestGame(RelativeLayout):
    segmentos = 10.0
    labeled = NumericProperty(1)
    segment_size = (Window.width/segmentos, Window.height/segmentos)
    def __init__(self, **kwargs):
        super(TestGame, self).__init__(**kwargs)
        self.StandUP()
        self.init_adminbar()
        self.labels = []
        for positem in self.Stand: 
            labelitem = Button(font_size = 8,font_name = font+ "Minecraftia-Regular.ttf",pos = positem, background_normal = color + "None.png", text = ""+str(positem), size_hint = (None,None), size = (45,8), valign = 'bottom', halign = 'left')
            labelitem.texture_update() 
            labelitem.text_size = labelitem.texture_size
            labelitem.size = labelitem.texture_size
            self.labels.append(labelitem)
            self.add_widget(labelitem)
        self.add_widget(Button(pos = (320,420),background_normal = color + "5.png" , size_hint = (None,None), size = self.segment_size))
        self.add_widget(Button(pos = (320,480),background_normal = color + "5.png" , size_hint = (None,None), size = self.segment_size))
        self.objeto = Button(pos = (0,360),background_normal = color + "6.png" , size_hint = (None,None), size = self.segment_size)
        self.add_widget(self.objeto)
        self.stand_segments()
        self.lanzar_cuadrito()
        #Clock.schedule_interval(lambda x: self.lanzar_cuadrito(), 30.0)
    
    def lanzar_cuadrito(self):
        print "he lanzado un cuadrito"
        projectil = Projectil(pos = (320,540) ) 
        self.add_widget(projectil)
        
    def stand_segments(self):
        v = 2 * 1/75 
        D1 = v * 60/2
        D2 = v * 180/2
        D3 = v * 320/2
        x = Animation(pos = (240, 360), d = 1 ) 
        x += Animation(pos = (240, 240), d = 1)
        x += Animation(pos = (400, 240), d = 1)
        x += Animation(pos = (400, 420), d = 1)
        x += Animation(pos = (720, 420), d = 1)
        x.repeat = True
        x.start(self.objeto)
        
    def init_adminbar(self):
        grid = GridLayout(cols = 1)
        buttongrid = GridLayout(rows = 1,size_hint_y = None, height = 27)
        with buttongrid.canvas:
            Color(1,0,0,1)
            Rectangle(size = (Window.width , 27), pos = (0,Window.height - 27))
        buttongrid.add_widget(Button(text = "Labeldpos",on_release = lambda x: self.switch_labeled(), background_normal = color + "3.png", background_down = color + "2.png"));
        grid.add_widget(buttongrid)
        grid.add_widget(Label())
        self.add_widget(grid)
    def switch_labeled(self): self.labeled *= -1
    def on_labeled(self,value,value1):
        self.labeled = int(self.labeled)
        print value1
        if value1 == 1:
            x = Animation(opacity = 1, d = .031250)
            for label in self.labels: x.start(label)
        else:
            x = Animation(opacity = 0, d = .031250)
            for label in self.labels: x.start(label)
    ...
    def StandUP(self):
        ...
        points_x = []
        points_y = []
        for i in range(int(self.segmentos)):
            points_x.append((Window.width/self.segmentos)*i)
            points_y.append((Window.height/self.segmentos)*i)
        for x in points_x:
            for y in points_y:
                self.Stand.append((x,y))
        with self.canvas:
            for i in range(int(self.segmentos)):
                i_x = Window.width/self.segmentos
                i_y = Window.height/self.segmentos
                Line(points=[ i_x * (i)       , 0                         , i_x * (i)   ,Window.height ], width=.25)
                Line(points=[ 0               , i_y * (i)               , Window.width  , i_y * (i)    ], width=.25)
