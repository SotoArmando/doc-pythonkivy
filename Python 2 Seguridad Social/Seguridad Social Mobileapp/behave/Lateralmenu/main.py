...
class Combine(RelativeLayout):
    def __init__(self, **kwargs):
        ...
        self.bttntext = Label(size_hint_y = None , text = kwargs["text"], markup = True, height = 30)
...
class LateralMenu(RelativeLayout):
    def __init__(self, **kwargs):
        ...
        self.stats = [1,1,1,1]
        self.stats1 = [1,1,1,1]
        pngs = ["heart","music","play-button","share"]
        grid = GridLayout(cols = 1,size_hint = (None,1), width = '94dp',spacing = 24,pos_hint = {'center_x':.5})
        ...
        for i in range(1):
            ...
            for i in range(4):
                a = Combine(source = pngs[i] + ".png", text = "[color=#000000]"+pngs[i])
