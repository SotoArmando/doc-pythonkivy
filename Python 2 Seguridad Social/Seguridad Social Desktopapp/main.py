...
class Interface(RelativeLayout):
    def __init__(self, **kwargs):
        ...
        topnavbar_root = GridLayout(cols = 1)
        topnavbar = TopNavigationS(); 
        topnavbar.y = Window.height - topnavbar.height
        ...
        self.pariente.height =  Window.height -topnavbar.height
        self.load_map()
        ...
        parienteroot.add_widget(topnavbar_root)
        topnavbar_root.add_widget(topnavbar)
        topnavbar_root.add_widget(Label())
        controler1 = GridLayout(cols = 1, size_hint = (None,None), width = dp(250), height = dp(125), pos_hint = {'center_x':.5}, y = dp(54))
        self.pos_status = Button(opacity = 1,text_size = (dp(250),dp(50)),background_normal = color + "T50.png",text = "John F. Kennedy 88, Buenaventura Freites", halign = 'center', valign = 'middle')
        ...
        desktopcontrols = GridLayout(rows = 1,size_hint_y = None, height = dp(54))
        desktopcontrols.add_widget(Button(background_normal = color + "2.png",background_down = color + "2.png"))
        texts = ["Estadisticas por sectores", "Actividad Policial","Actividad Demandada", "Coordinacion policiaca", "Medidas de Supervision"]
        for i in range(5): desktopcontrols.add_widget(Button(background_normal = color + "2.png",markup = True,text ="[b]"+texts[i],text_size = (dp(150),dp(54)), size_hint_x = None , width = dp(150), valign = 'middle', halign = 'center'))
        desktopcontrols.add_widget(Button(background_normal = color + "2.png",background_down = color + "2.png"))
        self.main_screen.add_widget(desktopcontrols)
        ...
        dim.add_widget(Button(opacity = 1,text_size = (dp(200),dp(50)),background_normal = color + "T50.png",text = "Armando Jose Soto Melo", halign = 'center', valign = 'middle'))
        ...
        if self.sc.current == "pariente":
            ...
            for i in self.coneccion.tables_data[2][0]:
                try: 
                    ...
                    self.inmap.append(i)
                    print "NOTIFICAR"
                    a = RelativeLayout(opacity = 0,size_hint = (None,None), size = (200,150), pos_hint = {'center_x':.85})
                    self.add_widget(a)
                    a.add_widget(Button(background_normal = color + "3.png",background_down = color + "3.png",text = "[b]Nueva Alarma[/b]\nAv.Jose Contreras #78",markup = True))
                    x = Animation(opacity = 1, d = 2, t = 'out_expo') +Animation(opacity = 0, d = 2, t = 'in_expo')
                    x.start(a)
        else:
            pass
    def load_map(self,**kwargs):
        ...
        for i in self.data[2][0]:
            ...
            subdim = RelativeLayout(size_hint_x = None, width = dp(50))
            subdim.add_widget(Image(source = color + "T50.png", keep_ratio = False , allow_stretch = True))
            subdim.add_widget(ImageButton(on_release = lambda x: self.dibujarmisdirecciones(str((x.parent.parent.parent.parent.lat, x.parent.parent.parent.parent.lon))[1:-1].split(',')), source = asset + "ic_directions_white_36dp.png"))
            dim.add_widget(subdim)
            marcador.add_widget(dim)
            self.x_mapview.add_marker(marcador)
            self.inmap.append(i)
